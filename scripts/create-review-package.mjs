...
function main() {
   ...
   *
   * A product/UX review cannot be done from prose, so the deterministic screenshots and the
   * before/after document travel with the package. They are binaries, so they are read and appended
   * rather than passed through the text path above. `review/screenshots/` is generated by
   * `npm run screenshots` from the sealed demo archive — no real data can reach it, because the
   * database it renders was built from a file in this repository.
   */
  const shotsDir = join(ROOT, 'review', 'screenshots');
  if (existsSync(shotsDir)) {
    for (const file of readdirSync(shotsDir).sort()) {
      if (!file.endsWith('.png')) continue;
      entries.push({
        name: `review/screenshots/${file}`,
        data: readFileSync(join(shotsDir, file)),
      });
    }
  }
  const beforeAfter = join(ROOT, 'review', 'UX_BEFORE_AFTER.md');
  if (existsSync(beforeAfter)) {
    entries.push({ name: 'review/UX_BEFORE_AFTER.md', data: readFileSync(beforeAfter) });
  }
...
Product and UX production readiness. The data layer is untouched: no schema change, no backup-format
change, no change to merge, restore or integrity semantics, and no new runtime dependency of any kind.
What changed is the interface around it, driven by an audit of the running application against a
31-record demo dataset rather than against an empty database
(\`docs/phase-2-ux-audit.md\`).

**Layout.** One 1360 px measure for every view became a measure per view: 概览/工作/荣誉/设置 grow to
1560, 台账 to 1920, 报告 stays at 1180. At 1920 the ledger went from 1328 px of table in a 1920 px
viewport to 1850 px.

**Shell.** Two full-width bands (a 56 px red bar carrying only the title, plus a 45 px navigation
strip) became one 48 px band carrying the wordmark, the six destinations and the current view's
primary action. Brand red is now a 3 px rule, the wordmark and the active underline.

**工作 — the phase's only P0.** A ~115 px card per record put six of twenty-four on a 1920×1080
screen, with nothing aligned and the expand control ~1200 px from the title. Desktop now renders a
42 px row on a shared column template, sixteen to a screen, with the whole row as the disclosure
control; below 72rem the same component still renders the stacked card.

**台账 — the other P0.** Automatic table layout sized 类别 so its two-character value wrapped to two
lines on every row. Declared column widths, zebra striping and a sticky header (where the wrapper is
not a scroll container) replace it.

**概览.** Six equal tiles — one of them the union of three others — became three attention counts and
the follow-up list that was already the best thing on the page. An empty database now gets a first-run
page instead of six zeroes.

**Empty states.** "Nothing exists" and "your filter excluded everything" are now different sentences
with different offers.

**荣誉.** The 业务分类 filter, which could only ever return an empty list on this view, was removed;
the counterpart filter is named 授予单位, which is what it filters.

**设置.** A sticky section index at wide widths, so 回收站 is reachable without scrolling past thirteen
category rows.

Bundle cost: initial payload +1.7% gzip (+3 327 bytes), no framework added.
\`review/UX_BEFORE_AFTER.md\` carries the measurements and the screenshots they refer to.
