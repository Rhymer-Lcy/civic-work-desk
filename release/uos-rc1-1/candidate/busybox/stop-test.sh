...
# Wait for it to go away, then escalate once.
#
# Ten iterations of `sleep 1` is up to ten seconds. POSIX `sleep` takes whole seconds only — sub-second
# waits would need a bashism or an external tool — so ten seconds is what the loop actually costs, and
# the comment now says so. The behaviour is left alone: a static file server with no open transfers
# exits on the first TERM, so the loop almost always ends on its first iteration, and a generous
# ceiling is the right trade against escalating to KILL too early.
i=0
while [ "$i" -lt 10 ]; do
  if ! kill -0 "$PID" 2>/dev/null; then
    rm -f "$PID_FILE"
