...
#   * the HTTP response evidence  (.runtime/headers-*.txt, from capture-http-evidence.sh)
#
# The response evidence includes the traversal probes and their safety assertions, so it is the file
# that carries the security result as well as the MIME result.
#
...
add ""
add "ENCODING NOTE: sections are concatenated byte for byte, so if one source file is not UTF-8"
add "the collected file will contain mixed encodings. This is expected and harmless — read it as"
add "bytes, or with errors=replace, rather than assuming one encoding. (Observed during the"
add "development rehearsal, where Python wrote its log in the console codepage.)"
...
for capture in "$BUNDLE_DIR"/.runtime/headers-*.txt; do
  [ -f "$capture" ] || continue
  append_file "HTTP RESPONSE EVIDENCE (含遍历探测)" "$capture"
  FOUND_EVIDENCE=yes
done
if [ "$FOUND_EVIDENCE" = no ]; then
  add ""
  add "WARNING: no HTTP response evidence found."
  add "Run: sh scripts/capture-http-evidence.sh busybox   (or ... python)"
  add "MIME types and the path-traversal result both come from that file; without it those rows of"
  add "the result sheet cannot have been filled in from measurement."
fi
