...
1. **第十二节「收集证据」必须在第十三节「卸载」之前。**
   卸载会删掉程序文件、菜单项、运行状态和部署日志——也就是收集脚本要采集的全部内容。
   反过来做，收集到的只会是一份「未安装」，最有力的自动化证据就没了。
2. **第十节「端口冲突」会自动把服务恢复成运行状态**，所以第十一节之后才能正常测 stop。

## 零、准备

需要三个文件（两个包，各自带一个校验值）：

- `civic-work-desk-uos20-loongarch64-<版本>.tar.gz` + `.sha256`（安装包）
- `civic-work-desk-uos20-final-acceptance-<版本>.tar.gz` + `.sha256`（本验收包）
...
**两个包都要校验。** 验收包自己也是要信任的东西：如果它在传输中损坏了，后面所有脚本的
结论都不可信。

```sh
cd <两个包所在目录>
sha256sum -c civic-work-desk-uos20-loongarch64-<版本>.tar.gz.sha256
sha256sum -c civic-work-desk-uos20-final-acceptance-<版本>.tar.gz.sha256
```

两条都必须显示 `OK`。任何一条不是 `OK` 就停下，不要解压，重新取一份。
把两条的实际输出都抄进结果表。
...
tar -xzf civic-work-desk-uos20-final-acceptance-<版本>.tar.gz

cd civic-work-desk-uos20-loongarch64-<版本>       && sha256sum -c SHA256SUMS.txt | tail -n 3 ; cd ..
cd civic-work-desk-uos20-final-acceptance-<版本>  && sha256sum -c SHA256SUMS.txt | tail -n 3 ; cd ..
```

（安装程序自己也会再校验一遍安装包，这一步是让你亲眼看到结果。）
...
```sh
$HOME/.local/bin/civic-work-desk-status
```

> 本手册里的命令都写成 `$HOME/.local/bin/...` 的完整路径。
> 这不是啰嗦：`~/.local/bin` 不一定在 PATH 里，直接敲 `civic-work-desk-status`
> 可能报「找不到命令」，那是 PATH 的问题，不是产品的问题——会造成误判。
...
```sh
$HOME/.local/bin/civic-work-desk-stop
$HOME/.local/bin/civic-work-desk-status      # 应报告未运行
$HOME/.local/bin/civic-work-desk             # 重新启动并打开
```
...
**脚本最后会把 CivicWorkDesk 的正常服务重新启动起来并做一次健康检查**，所以做完这一节，
服务是运行状态，下一节可以正常测停止。脚本最后会打印 `RESULT: PASS` 或 `RESULT: FAIL`，
把整段输出保留下来。
...
3. 确认：status 显示新版本号，**而刚才录入的假数据仍在**；
4. 确认地址仍然是 `http://127.0.0.1:8765/`；
5. `$HOME/.local/bin/civic-work-desk-rollback`，确认切回旧版本，数据仍在。
...
**然后把服务重新启动起来**，下一节的证据收集要在服务运行的状态下做：

```sh
$HOME/.local/bin/civic-work-desk --no-browser
```

## 十三、收集证据（第 14 项）——**必须在卸载之前做**
...
它会在当前目录生成一个 `civic-work-desk-final-results-<时间>.txt`，内容只包括：
状态输出、版本信息、部署日志、服务日志、菜单项内容、以及安装目录的文件清单。

**它不会**读取浏览器配置、浏览历史、Cookie、下载的文件，也不会列出你的主目录。

**把这个文件先保存好、复制到验收包目录之外**（例如你的家目录），再做下一节。
下一节的卸载会删掉它采集的那些东西；文件本身不会被删，但为稳妥起见先留一份。
...
然后跑一个只做卸载后检查的小脚本，它**不会**覆盖上一节收集到的证据文件：

```sh
sh scripts/post-uninstall-check.sh
```

它另存一个 `civic-work-desk-post-uninstall-<时间>.txt`，逐项核对「该删的删了、不该删的还在」。
...
- [ ] `civic-work-desk-final-results-<时间>.txt`（第十三节，卸载前收集的）
- [ ] `civic-work-desk-post-uninstall-<时间>.txt`（第十四节）
- [ ] 第四节 Console 输出的那段 JSON
- [ ] 端口冲突脚本的完整输出
- [ ] 失败项的截图（**只有失败时才需要**）
...
判断服务是否正常，看 status 的健康检查结果，不要看日志里有没有内容。
